"""email-temp.com provider implementation (HTML scraping with surl cookie).

Same backend family as emailfake.com and generator.email.
Different domain list and URL.
"""
...
class EmailTempProvider(TempMailProvider):
    """Provider for email-temp.com — surl cookie + channel URL, HTML scraping."""

    BASE_URL = "https://mail-temp.com"
    COOKIE_DOMAIN = ".mail-temp.com"
    ...
    def _get_domains(self) -> list[str]:
        resp = self._http.get(f"{self.BASE_URL}/temp-mail-box/", timeout=30)
        ...
        if not domains:
            for text in soup.stripped_strings:
                if re.match(r"^[a-z0-9-]+\.[a-z.]{2,}$", text) and "mail-temp" not in text:
                    domains.append(text)
        if not domains:
