"""generator.email provider implementation (HTML scraping with surl cookie).

Same backend as emailfake.com — shares the same gasmurl pattern, Socket.IO,
and del_mail.php endpoints. Different domain list and UI language.
"""
...
class GeneratorEmailProvider(TempMailProvider):
    ...
    def _get_domains(self) -> list[str]:
        ...
        if not domains:
            for text in soup.stripped_strings:
                if re.match(r"^[a-z0-9-]+\.[a-z.]{2,}$", text) and "generator" not in text:
                    domains.append(text)
        if not domains:
    ...
    def get_inbox(self, email: str) -> list[Message]:
        ...
        # generator.email shows inbox at /{email} URL
        resp = self._http.get(f"{self.BASE_URL}/{email}", timeout=30)
        if resp.status_code != 200:
