...
| Language | Rust 1.70+ |
| Runtime | Cargo |
| Dependencies | `reqwest`, `serde`, `scraper` (for HTML scraping) |
| Build tool | Cargo |
...
```bash
cargo test --test e2e 1secemail
cargo test --test e2e mail_tm
cargo test --test e2e guerrilla_mail
cargo test --test e2e yopmail
cargo test --test e2e dropmail
cargo test --test e2e zoromail
cargo test --test e2e tempmail_lol
```
...
- **YOPmail** — HTML scraping via `scraper`; no official API. `delete_email` always returns `true` (no delete endpoint).
...
1. Run the full suite: `cargo test --test e2e`
