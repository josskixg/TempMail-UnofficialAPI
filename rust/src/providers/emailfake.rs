...
const BASE_URL: &str = "https://emailfake.com";
/// Used to exclude the site's own domain when scanning for domain-like tokens.
const SITE_KEY: &str = "emailfake";
...
impl Emailfake {
    ...
    async fn get_domains(&self) -> Result<Vec<String>, TempMailError> {
        ...
        let mut domains: Vec<String> = re
            ...
            .collect();
        if domains.is_empty() {
            // ponytail: fallback — scan visible text for domain-like tokens
    ...
    /// Load a channel page with the surl cookie identifying our mailbox.
    async fn fetch_inbox_html(&self) -> Result<String, TempMailError> {
        ...
        // ponytail: surl cookie is the mailbox selector; inject via header, no cookie jar needed
        let resp = self
            .client
            .inner()
            .get(&url)
...
#[async_trait]
impl TempMailProvider for Emailfake {
    ...
    async fn get_inbox(&self, _email: &str) -> Result<Vec<Message>, TempMailError> {
        ...
        // ponytail: regex over the whole page — list-group-item anchors only live in #email-table
    ...
    async fn read_message(&self, message_id: &str) -> Result<MessageDetail, TempMailError> {
        ...
        // ponytail: non-greedy to first </div>; nested email HTML may truncate — fallback to whole page
