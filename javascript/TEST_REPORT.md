...
| Dependencies | `node-fetch` or built-in `fetch` (Node 18+), `cheerio` (for HTML scraping) |
...
node --test --test-name-pattern="zoromail" tests/e2e.test.js
node --test --test-name-pattern="tempmail.lol" tests/e2e.test.js
```
